...
function resolveWikiTarget(fromPath: string, rawTarget: string): string[] {
  ...
  const candidates = [
    normalizeWikiTarget(join(fromDir, target)),
    target,
  ];
...
for (const page of inbound.keys()) {
  ...
  if ((inbound.get(page)?.size ?? 0) === 0) bad.push(`memory/wiki/${page}.md: no inbound wiki links`);
}
...
for (const heading of logHeadings) {
  if (!/^\[\d{4}-\d{2}-\d{2}\]\s+\S+\s+\|\s+.+$/.test(heading)) {
    bad.push(`memory/wiki/log.md: unparseable log heading ${heading}`);
  }
}
