...
type Source = { path: string; title: string; tags: string[]; kind: string; date?: string; source?: string };
...
function table(sources: Source[]): string {
  return [
    "| Source | Kind | Tags |",
    "|---|---|---|",
    ...sources.map((s) => `| [${s.title.replaceAll("|", "\\|")}](${relative(dirname("memory/wiki/sources/index.md"), s.path).replaceAll(" ", "%20")}) | ${s.kind} | ${s.tags.slice(0, 8).map((t) => `#${t}`).join(" ")} |`),
  ].join("\n");
}
...
const tagEntries = [...byTag.entries()].sort((a, b) => b[1].length - a[1].length || a[0].localeCompare(b[0]));
...
console.log(`backfilled ${sources.length} sources, ${tagEntries.length} tags (${materializedTagEntries.length} materialized pages)`);
